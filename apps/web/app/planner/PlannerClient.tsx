...
import { FeatureReadOnlyBanner } from "@/components/billing/FeaturePaywall";
  ...
  return (
    ...
    >
      <Checkbox
        ...
        aria-label={
          isReadOnly
            ? "Day completion is Pro-only — upgrade to resume tracking"
            : undefined
        }
        className="mt-1"
      />
      ...
      <div className="flex items-center gap-2 shrink-0">
        {/* DayActionButton routes to practice flows — still allowed for
            free-tier read-only users since those downstream flows have
            their own gating (behavioral + technical are free; resume +
            planner jumps land on the paywall / login). */}
...
/**
 * Interactive Planner UI. Accepts `isReadOnly` so the same component can be
 * reused as the grandfathered read-only surface for free-tier users who had
 * plans before the Pro gate landed — the New Plan / archive / day-toggle
 * affordances are hidden, but the list + detail view remain.
 */
export default function PlannerClient({ isReadOnly = false }: { isReadOnly?: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-6xl mx-auto">
      ...
      {isReadOnly && <FeatureReadOnlyBanner feature="planner" />}
